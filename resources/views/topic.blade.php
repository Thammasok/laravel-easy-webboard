...
@section('content')
  <div class="container">
    ...
    <section class="section">
      <div>
        <a href="{{ url('/new') }}" class="button is-primary"> ตั้งกระทู้ใหม่ </a>
      </div>
      <br>
      <hr>
      <!-- <article class="media">
            ...
            <div class="column is-three-quarters">
              <div>
                ...
                <div style="display: inline; padding-left: 10px;">
                  <a href="{{ url('/') }}/{{ $topic['id'] }}" class="title is-5">{{ $topic['subject'] }}</a>
                  <br>
        ...
        <ul class="pagination-list">
          @if (!empty($data['topics']['page_all'] && $data['topics']['page_all'] > 1))
            @for ($i = 1; $i <= $data['topics']['page_all']; $i++)
              <li>
                @if ($i == $data['page'])
                  <a href="{{ url('/') }}?page={{ $i }}" class="pagination-link is-current">{{ $i }}</a>
                @else
                  <a href="{{ url('/') }}?page={{ $i }}" class="pagination-link">{{ $i }}</a>
                @endif
              </li>
            @endfor
          @endif
